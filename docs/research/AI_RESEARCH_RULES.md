...
4. **Prefer summaries over full reports.** Read `summaries/` first, always. Open
...
6. **Compare alternatives.** Never present a single option as though it were the
...
7. **Include costs.** Money, compute, storage, and human time. Estimates are
...
8. **Include maintenance.** Who keeps this alive? What breaks when the upstream
...
9. **Include implementation details.** A recommendation that cannot be acted on
...
10. **Cite sources.** Every non-obvious factual claim needs a source: paper,
...
11. **Identify uncertainty.** State clearly what you do not know, what you could
...
12. **Every full report ships with a ≤2-page summary.** The summary is not a
...
13. **Write for a stranger in eighteen months.** No unexplained references to
    "the earlier discussion." No assumed context. Documents must stand alone.

14. **Numbers over adjectives.** "Fast", "cheap", "high-quality", and "scalable"
    are not findings. Latencies, dollar figures, benchmark scores, and parameter
    counts are.

15. **Record negative results.** "No usable Tigrinya dataset exists for this
...
16. **Separate what you found from what you recommend.** Findings are evidence;
...
2. Read this file
3. Read docs/decisions/DECISIONS.md and assumptions.md
4. Scan docs/research/summaries/ for prior work on your topic
5. Identify which stage you are in: Scout, Analyst, or Architect
6. Use the matching template from docs/research/templates/
7. Produce your output AND its summary
8. Record any new decision, rejection, or assumption
```
