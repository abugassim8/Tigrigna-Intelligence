...
items), seven enforcement scripts, a **planted-failure suite** (102 cases, CI),
...
| **GAP-5** | **The MVP is incomplete by its own definition** | ✅ **Measurement half closed 2026-09-08, and widened to the whole anchor 2026-09-11.** All five intrinsic checks ran over **all 9,212 Tigrinya segments of TICO-19** (194,588 word tokens): `surface`, `alignment` and `determinism` at **100%**, `coverage` **59.91%** (lower bound) and `normalisation` **73.10%** recorded as MEAS. The `metrics.md` row is no longer ❌. ⚠️ **The gap is not fully closed:** a clean `pip install` still cannot analyse morphology — DEC-028 working as intended, not a defect — so CI still reports SKIP, and **accuracy** remains unmeasured because it needs gold data and a speaker (**A-13**). ⚠️ The full anchor also found what the sample could not: normalisation **destroys** an analysis in 2 cases (the sample said 0). Numbers and their weaker-than-usual guarantee: `docs/benchmarks/measurements/` |
...
test_plants.py`, 102 cases, in CI). That is the response to a discipline that
