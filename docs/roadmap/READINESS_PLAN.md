...
| **GAP-5** | **The MVP is incomplete by its own definition** | ✅ **Measurement half closed 2026-09-08.** HornMorpho was installed and all five intrinsic checks ran against a real analyser: `surface`, `alignment` and `determinism` at **100%**, `coverage` **62.27%** (lower bound) and `normalisation` **75.61%** recorded as MEAS. The `metrics.md` row is no longer ❌. ⚠️ **The gap is not fully closed:** a clean `pip install` still cannot analyse morphology — that is DEC-028 working as intended, not a defect — so CI still reports SKIP, and **accuracy** remains unmeasured because it needs gold data and a speaker (**A-13**). Numbers and their weaker-than-usual guarantee: `docs/benchmarks/measurements/` |
