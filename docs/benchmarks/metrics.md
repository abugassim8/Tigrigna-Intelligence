...
> ✅ **Morphology joined them 2026-09-08** — HornMorpho was installed and all five intrinsic checks ran against a real analyser for the first time. Three PASS at 100%, two are recorded as **MEAS** (no threshold; the first measurement *sets* a floor, it is not judged by one).
...
| Morphological analysis | Consistency + coverage *(accuracy needs gold data)* | **Yes — intrinsic** | **coverage ≥62.27%**, first measurement | ✅ **Measured 2026-09-08**, the first time any morphological property of Tigrinya was measured here. `surface` **899/899**, `alignment` **899/899**, `determinism` **4,435/4,435** — all 100%. **`coverage` 10,668/17,133 = 62.27%**, a **lower bound** (an analysis identical to the surface counts as uncovered, which also catches genuinely uninflected words). **`normalisation` 31/41 = 75.61%**: of 72 words that change under ጸ/ፀ · ኣ/አ normalisation, 31 are unanalysable either way and excluded; of the 41 informative pairs **31 unchanged, 7 rescued** (analysable only *after* normalising — it is working), **0 lost**, **3 differ**. ⚠️ **Measured on a 900-segment sample, not the anchor** — HornMorpho runs at ~0.85 s/word, so the full anchor is ~40 h. ⚠️ **CI cannot re-derive this** (GPL-3.0, 4.1 GB, hours): weaker guarantee than any `experiments/` entry, recorded in `measurements/README.md`. ⚠️ **P-4 is cleared for *consistency*, not accuracy** — these checks catch **broken**, not **wrong**. *History: this row read "Yes — intrinsic" citing exp 004 until 2026-08-22; experiment 004 never tested morphology. It was ❌ from then until today.* | 
